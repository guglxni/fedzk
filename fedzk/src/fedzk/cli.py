...
def generate_command(args):
    ...
    try:
        ...
        if not circuit_path.exists() or not proving_key_path.exists():
            print(f"Error: Required ZK files not found. Run 'fedzk setup' first.")
            return 1
...
def verify_command(args):
    ...
    try:
        ...
        if not verification_key_path.exists():
            print(f"Error: Verification key not found. Run 'fedzk setup' first.")
            return 1
        ...
        if args.batch or proof_result.get("is_batch", False):
            ...
            if verified:
                print(f"✅ Batch proof VERIFIED successfully!")
                ...
                print(f"❌ Batch proof verification FAILED!")
                if details['merkle_root_verified']:
                    print(f"   - Merkle root verified: Yes")
                    print(f"   - Some chunk proofs failed verification")
                else:
            ...
            if verified:
                print(f"✅ Secure proof VERIFIED successfully!")
            else:
                print(f"❌ Secure proof verification FAILED!")
            ...
            if verified:
                print(f"✅ Standard proof VERIFIED successfully!")
            else:
                print(f"❌ Standard proof verification FAILED!")
