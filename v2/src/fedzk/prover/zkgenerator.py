...
def _flatten_pad_or_truncate(values: List[int], max_inputs: int, context: str) -> List[int]:
    """Pad or truncate integer field elements to circuit capacity with honest logging."""
    if len(values) > max_inputs:
        print(
            f"⚠️  FEDzk: truncating {len(values)} → {max_inputs} gradients ({context}). "
            f"N_dev={max_inputs}; see PLAN Phase 1 / CONCEPTS P4 honest capacity."
        )
        return values[:max_inputs]
    if len(values) < max_inputs:
        print(
            f"⚠️  FEDzk: padding {len(values)} → {max_inputs} gradients with zeros ({context})."
        )
        return values + [0] * (max_inputs - len(values))
    return values
...
class ZKProver:
    ...
    def __init__(
        ...
        min_active: int = 1,
        scale_factor: int = DEFAULT_QUANT_SCALE,
        quantize: bool = True,
    ):
        """
        ...
        Args:
            ...
            max_norm_squared: Max allowed squared L2 norm in *float* space
                (converted to quantized domain via × scale_factor²)
            min_active: Minimum number of non-zero gradient elements required
            scale_factor: Quantization scale (float → integer field element)
            quantize: If True (default), run GradientQuantizer before witness gen
        """
        self.secure = secure
        ...
        self._quantizer = GradientQuantizer(scale_factor=scale_factor) if quantize else None
        self.last_quantization_metadata: Dict[str, Any] = {}
    ...
    def _quantize_flat(self, gradient_dict: Dict[str, torch.Tensor], max_inputs: int) -> List[int]:
        """Quantize then flatten to exactly max_inputs integer field elements."""
        if self._quantizer is None:
            flat: List[int] = []
            for grad_tensor in gradient_dict.values():
                flat.extend(int(round(float(x))) for x in grad_tensor.flatten().tolist())
            self.last_quantization_metadata = {"quantize": False}
            return _flatten_pad_or_truncate(flat, max_inputs, "unquantized-path")

        quantized, metadata = self._quantizer.quantize_gradients(gradient_dict)
        self.last_quantization_metadata = metadata
        flat = []
        for values in quantized.values():
            flat.extend(int(v) for v in values)
        return _flatten_pad_or_truncate(flat, max_inputs, "quantized-prove")
    ...
    def generate_real_proof_standard(self, gradient_dict: Dict[str, torch.Tensor], max_inputs: int = N_DEV):
    ...
    def generate_real_proof_secure(self, gradient_dict: Dict[str, torch.Tensor], 
                                 max_norm_sq: float, min_active_elements: int, max_inputs: int = N_DEV):
    ...
    def _prepare_input_standard(self, gradient_dict: Dict[str, torch.Tensor], max_inputs: int = N_DEV):
        """
        Prepare input data for the standard (basic) circuit.

        Quantizes float tensors to integer field elements (mandatory on prove path).
        Pads/truncates to max_inputs with an explicit warning (N_dev honesty).
        """
        gradients = self._quantize_flat(gradient_dict, max_inputs)
    ...
    def _prepare_input_secure(self, gradient_dict: Dict[str, torch.Tensor],
        ...
        max_norm_sq is interpreted in float space and scaled by scale_factor² so the
        circuit compares against quantized gradient magnitudes.
        """
        gradients = self._quantize_flat(gradient_dict, max_inputs)
        scale = self.scale_factor if self.quantize else 1
        max_norm_num = int(max_norm_sq * (scale ** 2))
        min_active_num = int(min_active_elements)
