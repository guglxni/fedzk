...
def doctor_command(
    ...
    """Check ZK toolchain, artifacts, and CLI integrity (Phase 0 sensor)."""
    from fedzk.doctor import format_doctor_report, run_doctor
...
def verify_command(
    ...
    if batch:
        ...
        vkey_name = "verification_key_secure.json" if secure else "verification_key.json"
        vkey_path = str(ASSET_DIR / vkey_name)
...
def client_prove_command(
    ...
    try:
        ...
        if circuit_type == "batch" or batch_size > 1:
            ...
            if gpu:
                typer.echo("⚠️  GPU flag ignored for BatchZKProver (not supported yet)")
            proof_result = prover.generate_proof(gradients)
        else:
            logger.info("Using standard ZK prover...")
            prover = ZKProver(secure=secure)
            if gpu:
                typer.echo("⚠️  GPU flag ignored for ZKProver (snarkjs CPU path)")
